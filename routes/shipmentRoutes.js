...
router.get("/years", handleAsyncErrors(getAvailableYears));
...
router.post("/cache/refresh", handleAsyncErrors(async (req, res) => {
  ...
  res.status(200).json({
    message: `Cache refresh initiated for: ${dataType}`,
    success: true
  });
}));
...
console.log('   - GET /api/shipment/years');
console.log('   - GET /api/shipment/cache/status');
console.log('   - POST /api/shipment/cache/refresh');
